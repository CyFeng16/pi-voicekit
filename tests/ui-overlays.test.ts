...
describe("TtsOnboardingOverlay — render", () => {
	test("wide width renders chrome + recommendation + hint row", () => {
		...
		const o = new TtsOnboardingOverlay({ systemLocale: "en", theme: noTheme }, (r) => {
			resolved = r;
		});
...
describe("TtsOnboardingOverlay — handleInput resolves once", () => {
	test("[enter] resolves with kind 'test'", () => {
		let resolved: any = null;
		const o = new TtsOnboardingOverlay({ systemLocale: "en" }, (r) => {
			resolved = r;
		});
		o.handleInput("\r");
	...
	test("subsequent input is ignored after resolve", () => {
		let count = 0;
		const o = new TtsOnboardingOverlay({ systemLocale: "en" }, (_) => {
			count++;
		});
		o.handleInput("\r");
		o.handleInput("m");
