...
```bash
bun -e "const fs=require('node:fs'); const d=Bun.YAML.parse(fs.readFileSync('.github/workflows/release.yml','utf8')); const s=d?.jobs?.publish?.steps??[]; const names=s.map((x)=>x.name??x.uses); const want=['Verify tag matches package.json version','Setup Bun','Extract release notes from CHANGELOG.md','Install dependencies','Typecheck + tests','Bump npm to latest (supports OIDC)','Publish to npm (Trusted Publishing + provenance)','Verify the packed tarball matches the published one','Publish the GitHub Release']; if (s.length!==11) throw new Error('expected 11 steps (1 checkout + 10 named), got '+s.length); for (const w of want) if (!names.includes(w)) throw new Error('missing step: '+w); console.log('yaml parses; steps:', s.length)"
...
bun -e "const fs=require('node:fs'); const d=Bun.YAML.parse(fs.readFileSync('.github/workflows/release.yml','utf8')); d.jobs.publish.steps.forEach((s,i)=>{ if (s.run) fs.writeFileSync('/tmp/release-run-'+i+'.sh', s.run); }); console.log('extracted', d.jobs.publish.steps.filter((s)=>s.run).length, 'run blocks')"
...
bun -e "const fs=require('node:fs');const d=Bun.YAML.parse(fs.readFileSync('.github/workflows/release.yml','utf8'));fs.writeFileSync('/tmp/gate.sh', d.jobs.publish.steps.find((s)=>s.name.startsWith('Verify the packed')).run)"
...
bun -e "const fs=require('node:fs');const d=Bun.YAML.parse(fs.readFileSync('.github/workflows/release.yml','utf8'));fs.writeFileSync('/tmp/plan-guard.sh', d.jobs.publish.steps.at(-1).run)"
